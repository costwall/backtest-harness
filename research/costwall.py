...
def evaluate(
    ...
    *,
    ...
    if n_trades == 0:
        move_med = move_p90 = net_mean = t_stat = float("nan")
    else:
        move = np.abs(ret_taken) * 1e4
        move_med = float(np.median(move))
        move_p90 = float(np.percentile(move, 90))
        net = dir_taken * ret_taken * 1e4 - cost_bps        # bps net на сделку
...
def _degenerate(label, source, horizon, cost_bps, n_total) -> CostWallResult:
    ...
    return CostWallResult(
        ...
        move_bps_med=nan, move_bps_p90=nan, net_mean_bps=nan, net_t_stat=nan,
        drift_bps=nan, verdict="INSUFFICIENT_DATA",
    )


_RESULTS_HEADER = (
    "| date | label | source | H | cost_bps | n_total | n_eff | n_trades | auc_test | "
    "move_med | move_p90 | net_bps | t | drift | verdict |\n"
    "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|\n"
)
